...
# Week 18: Random Forest from Scratch (School Dropout Prediction)
# Author: Ing. Arturo Javier Borbon Rojas


# 1 Decision Tree Implementation
# 1.1 Define the class NODE
class Node:
...
if __name__ == "__main__":
    ...
    np.random.seed(42)
    # Synthetic Data: [Attendance, Grades, Platform Logins]
    X_train = np.random.rand(100, 3) * 100 
    # Logic: If average is < 50, high risk of dropout (Class 1)
    y_train = (np.mean(X_train, axis=1) < 50).astype(int)

    X_test = np.random.rand(20, 3) * 100
    y_test = (np.mean(X_test, axis=1) < 50).astype(int)
